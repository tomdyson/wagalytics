...
/***/ (function(module, exports, __webpack_require__) {
	...
	$(document).ready(function () {
	    var el = document.getElementById('wagalytics-data');
	...
	function setup(token_url, view_id) {
	    ...
	    gapi.analytics.ready(function () {
	        ...
	        var dashboard = new Dashboard(view_id);
	        dashboard.refresh();
	    });
	}
	...
	var Dashboard = function () {
	    function Dashboard(view_id) {
	        ...
	        this.start_date_input.addEventListener('change', function () {
	            return _this.refresh();
	        });
	        this.end_date_input.addEventListener('change', function () {
	            return _this.refresh();
	        });
	        $(document.getElementById("export")).submit(function () {
	            $(document.getElementById('export-data')).val(JSON.stringify(_this.data));
	        });
	    ...
	    _createClass(Dashboard, [{
	        ...
	        key: 'sessionsLineChart',
	        value: function sessionsLineChart() {
	            var _this2 = this;
	
	            var id = 'sessions-line-chart-container';
	            this.setLoading(id);
	            this.getQuery({
	                dimensions: 'ga:date,ga:nthDay',
	                metrics: 'ga:sessions'
	            }).then(function (results) {
	                _this2.data['sessions'] = results.rows;
	                $(_this2.export_btn).prop('disabled', false);
	                $(_this2.export_btn).removeClass('button-longrunning-active');
	
	                var data1 = results.rows.map(function (row) {
	                    return row[2];
	                });
	                var labels = results.rows.map(function (row) {
	                    return row[0];
	                });
	                labels = labels.map(function (label) {
	                    return (0, _moment2.default)(label, 'YYYYMMDD').format('ll');
	                });
	        ...
	        key: 'popularPagesTable',
	        value: function popularPagesTable() {
	            var _this3 = this;
	
	            var id = 'popular-pages-table-container';
	            this.setLoading(id);
	            var queryData = {
	            ...
	            this.getQuery(queryData).then(function (results) {
	                ...
	                _this3.data['pages'] = Object.values(bars);
	                $(_this3.export_btn).prop('disabled', false);
	                $(_this3.export_btn).removeClass('button-longrunning-active');
	
	                var table = createTable(['Page URL', 'Views'], Object.values(bars));
	                var pager = paginateTable(table);
	        ...
	        key: 'topReferrersTable',
	        value: function topReferrersTable() {
	            var _this4 = this;
	
	            var id = 'top-referrers-table-container';
	            this.setLoading(id);
	            var queryData = {
	                'metrics': 'ga:pageviews',
	            ...
	            this.getQuery(queryData).then(function (results) {
	                _this4.data['referrers'] = results.rows;
	                $(_this4.export_btn).prop('disabled', false);
	                $(_this4.export_btn).removeClass('button-longrunning-active');
	
	                var table = createTable(['Source', 'Views'], results.rows);
	                var pager = paginateTable(table);
	                var container = document.getElementById(id);
	                container.innerHTML = '';
	                container.appendChild(table);
	...
	function createTable(headings, rows) {
	    ...
	    for (i; i < headings.length; ++i) {
	        var heading = headerRow.insertCell(i);
	...
	function paginateTable(table) {
	    ...
	    $pager.find('.prev a').bind('click', function (event) {
	    ...
	    $pager.find('.next a').bind('click', function (event) {
	        event.preventDefault();
	        currentPage += 1;
